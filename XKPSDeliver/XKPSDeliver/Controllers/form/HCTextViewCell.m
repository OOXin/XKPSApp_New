...
- (void)setupSubviews{
    ...
    //标题
    _titleView = [UILabel labelWithFont:GetFont(14) textColor:GetColor(grayColor)];
    [self.contentView addSubview:_titleView];
    
    //输入框
    _textView = [[UITextView alloc] init];
    _textView.textColor = GetColor(grayColor);
    _textView.font = GetFont(12);
    ...
    _placeHolderView.font = GetFont(12);
    ...
    [_textView.rac_textSignal subscribeNext:^(id x) {
        ...
        weak__placeHolderView.hidden = content.length > 0;
        
        if (weak_self.item.maxInputLength) {
            UITextRange *selectedRange = [weak__textView markedTextRange];
            //获取高亮部分
            UITextPosition *position = [weak__textView positionFromPosition:selectedRange.start offset:0];
            // 没有高亮选择的字，则对已输入的文字进行字数统计和限制
            if (!position) {
                if (content.length > weak_self.item.maxInputLength) {
                    weak__textView.text = [content substringToIndex:weak_self.item.maxInputLength];
//                    [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"请输入不超过%zd个字",weakself.item.maxInputLength]];
        ...
        weak_self.item.content = content;
    }];
