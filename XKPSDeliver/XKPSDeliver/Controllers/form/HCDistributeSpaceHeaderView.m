...
- (void)setupSubviews{
    //标题
    _titleView = [UILabel labelWithFont:GetFont(14) textColor:GetColor(blackColor)];
...
- (void)setupAccessoryView{
    ...
    if (_group.rightText || _group.rightImage) {
        ...
        [[self.accessoryBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
            [weak_self.accessoryBtn setTitle:weak_self.group.rightText forState:UIControlStateNormal];
            if (weak_self.group.opertion) {
                weak_self.group.opertion(x);
            }
        }];
        [weak_self.contentView addSubview:weak_self.accessoryBtn];
    }
    ...
    if (_group.switchOptions) {
        ...
        [[self.accessorySwitch rac_signalForControlEvents:UIControlEventValueChanged] subscribeNext:^(id x) {
            weak_self.group.switchFlag = weak_self.accessorySwitch.flag;
            if (weak_self.group.opertion) {
                weak_self.group.opertion(x);
            }
        }];
        [weak_self.contentView addSubview:weak_self.accessorySwitch];
    }
}
